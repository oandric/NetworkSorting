...
#define TESTS 3				// Number of tests with different stream sizes (max = 19)
...
int main(void)
{
	...
	max_file_t *maxfile = NetworkSorting_init();
	...
	// Create a file for writing results
		...
		if (simNetMAX){
			...
			for(k=0 ; k<CYCLES ; k++){
				...
				NetworkSorting_actions_t actions;
				actions.instream_inX = in_array;
				actions.outstream_oData = out_array;
				actions.param_array_size = ARRAY_SIZE;
				actions.param_stream_size = STREAM_SIZE;

				NetworkSorting_run(engine, &actions);
