...
int main(int argc, char* argv[])
{
	...
	// Stream sizes (number of arrays to be sorted one after another
	...
	unsigned long STREAM_SIZE;	// Number of arrays that are consecutively sorted inside one cycle
	...
	//====================================================================================================
	//	Sequential sorting on CPU
	//====================================================================================================

	if (simSeqCPU){

		printf("\nStart of sequential sorting on CPU:\n");
		...
		printf("\n\nSequential sorting on CPU completed.\nSorting time: %lu s, %lu ns \n\n", sec(t4, t5), nsec(t4,t5));
		if (izpis){
			for(i = 0; i < ARRAY_SIZE; i++) {
					printf("[%d]\ta: %d\n", i, b[i]);
			}
	...
	//====================================================================================================
	//	Network sorting on CPU
	//====================================================================================================

	if (simNetCPU){

		printf("\nStart of network sorting on CPU:\n");
	...
	//	clock_gettime(CLOCK_REALTIME, &t4);
		for(m=0 ; m<CYCLES ; m++){
			...
			for(n=0 ; n<STREAM_SIZE ; n++){
				...
				clock_gettime(CLOCK_REALTIME, &t1);
				//===========================================================
				// Algorithm of network sorting for calculation on the CPU
				//===========================================================
	...
	clock_gettime(CLOCK_REALTIME, &t5);
		...
		printf("Network sorting on CPU completed.\nSorting time: %lu s, %lu ns \n\n", s, ns);
		if (izpis){
			for(i = 0; i < ARRAY_SIZE; i++) {
					printf("[%d]\ta: %d\n", i, y[i]);
			}
